...
(async()=>{
 ...
 for(const city of ['Paraibuna','Mauá']){g.context.place=city;g.run("state.city=place;state.facility={product:'pasta',city:place};buyPastaFactory(place)");assert.equal(g.run('ensurePastaFactoryState(place).owned'),false);assert.ok(!g.run("productionSite('pasta',place).point"));}
 assert.equal(g.run('state.balance'),30000);assert.equal(g.run('state.pastaFactory.stock[0].id'),'old','Campinas save is preserved');
 g.run("state.pastaFactories={'Mauá':{owned:true,stock:[{id:'retired'}]},'Paraibuna':{owned:true,stock:[]}};state.trip={facilityArrival:{product:'pasta',city:'Paraibuna'},to:'Paraibuna'};state.truckPallets=[{id:'in-transit'}];retireExtraPastaFactories();retireExtraPastaFactories()");
 assert.equal(g.run('Object.keys(state.pastaFactories).length'),0);assert.equal(g.run("state.retiredPastaFactories['Mauá'].stock[0].id"),'retired');assert.equal(g.run('state.facility'),null);assert.equal(g.run('state.trip.facilityArrival'),undefined);assert.equal(g.run('state.truckPallets[0].id'),'in-transit');
 assert.equal(g.run("productionSites().filter(s=>s.product==='pasta').length"),1);
 g.run("state.trip=null;playerCalls=[{id:'legacy',product:'pasta',origin:'Mauá',status:'available'}]");assert.equal(g.run('availableProductionCalls().some(c=>c.id==="legacy")'),false);
 assert.equal(g.run('ownStockCalls().length'),1);
 ...
 console.log('PASS: retired factories, preserved Campinas stock and active cargo, grouping by location and 48-hour activity expiry.');
