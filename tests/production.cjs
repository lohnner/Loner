...
if(require.main===module)(async()=>{
 ...
 g.run("state=fresh();state.city='Campinas';state.facility={product:'pasta',city:'Campinas'};state.pastaFactory={owned:true,stock:[{id:'own-campinas',product:'pasta',origin:'Campinas',destination:'São Paulo',producedAt:now}],startedAt:now,readyAt:now+PASTA_GROW_MS};playerCalls=[];records.set('progress/owner',structuredClone(state))");
 await g.run("claimPlayerCall('own-campinas')");assert.equal(g.run('state.truckPallets.length'),1,'Own unpublished stock is published and loaded atomically');assert.equal(g.run("ensurePastaFactoryState('Campinas').stock.length"),0);assert.equal(g.run("records.get('playerCalls/own-campinas').status"),'claimed');
